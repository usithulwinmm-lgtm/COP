...
function renderStoredHistory() {
    ...
    // ၁။ Buy နှင့် Sell ဖြစ်သော data များကိုသာ filter လုပ်၍ စစ်ထုတ်ယူခြင်း
    const filteredHistory = history.filter(trade => {
        const side = trade.side.toLowerCase();
        return side === 'buy' || side === 'sell' || side === 'buy/long' || side === 'sell/short';
    });

    // ၂။ History မရှိလျှင် ပြသရန် logic
    if (filteredHistory.length === 0) {
        container.style.display = "none";
        toggleBtn.innerText = "View All";
        historyBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">No trade history available</td></tr>';
        return;
    }

    // ၃။ Filter လုပ်ထားသော Buy/Sell data များကိုသာ Table ထဲတွင် ထည့်သွင်းပြသခြင်း
    historyBody.innerHTML = filteredHistory.map(trade => `
        <tr>
            <td>${trade.date || trade.time}</td>
            <td>${trade.asset || trade.pair}</td>
            <td class="${trade.side.toLowerCase().includes('buy') ? 'green' : 'red'}">
                ${trade.side.toUpperCase()}
            </td>
            <td>${trade.price}</td>
            <td>${trade.amount || trade.riskAmount}</td>
            <td>${trade.total || (trade.price * trade.amount).toFixed(2)} USDT</td>
        </tr>
    `).join('');
}
