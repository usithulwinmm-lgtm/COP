...
function renderStoredHistory() {
    ...
    // Auto-Hide Logic: Hide if empty, show if has data
    if (history.length === 0) {
        container.style.display = "none";
        toggleBtn.innerText = "View All";
        historyBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">No history available</td></tr>';
        return;
    } else {
        // Optional: Keep it hidden until user clicks "View All" 
        // OR auto-show when a new trade is added.
        // container.style.display = "block"; 
        // toggleBtn.innerText = "Hide";
    }

    historyBody.innerHTML = history.map(trade => `
        <tr>
            <td>${trade.date}</td>
            <td>${trade.asset}</td>
            <td class="${trade.side === 'buy' ? 'green' : 'red'}">${trade.side.toUpperCase()}</td>
            <td>${trade.price}</td>
            <td>${trade.amount}</td>
...
// ၁။ Asset Dropdown ကို history ထဲမှာရှိတဲ့ coin တွေနဲ့ အလိုအလျောက် ဖြည့်ပေးရန်
function updateAssetDropdown() {
    const assetSelect = document.getElementById('filter-asset');
    if (!assetSelect) return;
    
    let history = JSON.parse(localStorage.getItem('tradeHistory')) || [];
    // ရှိသမျှ asset တွေကို unique ထုတ်ယူမယ်
    const assets = [...new Set(history.map(item => item.asset))];
...
function renderHistory() {
    ...
    // Filter values များယူခြင်း
    const typeFilter = document.getElementById('filter-type').value.toLowerCase();
    const assetFilter = document.getElementById('filter-asset').value;
    const statusFilter = document.getElementById('filter-status').value.toLowerCase();
    const timeFilter = document.getElementById('filter-time').value;

    // Filtering လုပ်ငန်းစဉ်
    let filteredData = history.filter(item => {
        // --- အရေးကြီးသောအပိုင်း ---
        // Side/Type မှာ 'buy' သို့မဟုတ် 'sell' ဖြစ်မှသာ လက်ခံမယ် (Deposit, Withdraw များကို ပယ်ထုတ်ခြင်း)
        const side = item.side ? item.side.toLowerCase() : '';
        if (side !== 'buy' && side !== 'sell') return false;

        // ၁။ All type dropdown အတွက် Filter (Buy/Sell ရွေးချယ်မှု)
        const matchType = (typeFilter === 'all') ? true : side === typeFilter;
        
        // ၂။ Asset Dropdown Filter
        const matchAsset = (assetFilter === 'all') ? true : item.asset === assetFilter;
        
        // ၃။ Status Dropdown Filter (Success/Completed သာပြရန်)
        const matchStatus = (statusFilter === 'all') ? true : 'completed' === statusFilter;

        // ၄။ Time Dropdown Filter
        let matchTime = true;
        if (timeFilter !== 'all') {
            const tradeDate = new Date(item.date);
            const now = new Date();
            const diffDays = (now - tradeDate) / (1000 * 60 * 60 * 24);
            matchTime = diffDays <= parseInt(timeFilter);
        }

        return matchType && matchAsset && matchStatus && matchTime;
    });

    // ဇယားထဲသို့ HTML ထည့်သွင်းခြင်း
    if (filteredData.length === 0) {
        historyBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px; color: #848e9c;">No trade history available</td></tr>';
    } else {
        historyBody.innerHTML = filteredData.map(trade => `
            <tr style="border-bottom: 1px solid #2b3139;">
                <td style="padding: 12px;">${trade.date}</td>
                <td style="padding: 12px;">${trade.asset}</td>
                <td style="padding: 12px;" class="${trade.side.toLowerCase() === 'buy' ? 'text-green-500' : 'text-red-500'}">
                    ${trade.side.toUpperCase()}
                </td>
                <td style="padding: 12px;">${parseFloat(trade.price).toLocaleString()}</td>
                <td style="padding: 12px;">${parseFloat(trade.amount).toFixed(4)}</td>
                <td style="padding: 12px; color: #eaecef;">${parseFloat(trade.total).toLocaleString()} USDT</td>
            </tr>
        `).join('');
    }
}

// ၃။ window.onload ကို ပြင်ဆင်ရန်
window.onload = function() {
    updateAssetDropdown(); // Dropdown ဖြည့်မယ်
    renderHistory();       // Data ပြမယ်
    updateTopBalance();
};

// ၁။ Table Row ကို နှိပ်ရင် Details ပြပေးမည့် function
function showTradeDetails(index) {
    ...
    const sideColor = trade.side.toLowerCase() === 'buy' ? 'text-green-500' : 'text-red-500';
    ...
    content.innerHTML = `
        ...
        <div class="flex justify-between border-b border-gray-800 pb-2">
            ...
            <span class="${sideColor} font-bold uppercase">${trade.side}</span>
        </div>
        <div class="flex justify-between border-b border-gray-800 pb-2">
            <span class="text-gray-400">Price</span>
            <span class="text-white">${trade.price} USDT</span>
        </div>
        <div class="flex justify-between border-b border-gray-800 pb-2">
            <span class="text-gray-400">Amount</span>
            <span class="text-white">${trade.amount}</span>
        </div>
        <div class="flex justify-between">
            <span class="text-gray-400">Total</span>
            <span class="text-yellow-500 font-bold">${trade.total} USDT</span>
        </div>
    `;
...
// ၂။ Modal ပိတ်သည့် function
function closeTradeDetails() {
    document.getElementById('trade-details-modal').classList.add('hidden');
}

// ၃။ renderStoredHistory function ကို ပြင်ဆင်ရန် (Row မှာ onclick ထည့်ခြင်း)
function renderStoredHistory() {
    ...
    if (history.length === 0) {
        historyBody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">No history</td></tr>';
        return;
    }

    historyBody.innerHTML = history.map((trade, index) => `
        <tr onclick="showTradeDetails(${index})" class="hover:bg-gray-800 cursor-pointer transition border-b border-gray-800">
            ...
            <td class="p-3">${trade.asset}</td>
            <td class="p-3 ${trade.side.toLowerCase() === 'buy' ? 'text-green-500' : 'text-red-500'} font-bold uppercase">
                ${trade.side}
            </td>
            <td class="p-3">${trade.price}</td>
            <td class="p-3">${trade.amount}</td>
            <td class="p-3 text-white">${trade.total} USDT</td>
        </tr>
    `).join('');
}